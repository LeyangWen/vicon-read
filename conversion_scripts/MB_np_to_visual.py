...
def parse_args():
    ...
    parser.add_argument('--filter_bad_support_kpts', type=bool, default=True)
...
if __name__ == '__main__':
    ...
    if args.filter_bad_support_kpts or args.filter_lowConf_legs:
        # load pkl file generated by conversion_scripts/MB_np_detect_bad_est.py
        ...
        if args.filter_bad_support_kpts:
            # if mask, change value to nan for that frame
            mask = bad_kpts_data['mask']  # shape: (frame_num, 6) ['LSHOULDER', 'RSHOULDER', 'LELBOW', 'RELBOW', 'LHAND', 'RHAND']
            assert mask.shape[0] == estimate_pose.shape[0], f"mask.shape[0]: {mask.shape[0]}, estimate_pose.shape[0]: {estimate_pose.shape[0]}, they should be the same"
            estimate_skeleton.poses['LAP_b'][mask[:, 0]] = np.nan
            estimate_skeleton.poses['LAP_f'][mask[:, 0]] = np.nan
            estimate_skeleton.poses['RAP_b'][mask[:, 1]] = np.nan
            estimate_skeleton.poses['RAP_f'][mask[:, 1]] = np.nan
            estimate_skeleton.poses['LLE'][mask[:, 2]] = np.nan
            estimate_skeleton.poses['LME'][mask[:, 2]] = np.nan
            estimate_skeleton.poses['RLE'][mask[:, 3]] = np.nan
            estimate_skeleton.poses['RME'][mask[:, 3]] = np.nan
            estimate_skeleton.poses['LMCP2'][mask[:, 4]] = np.nan
            estimate_skeleton.poses['LMCP5'][mask[:, 4]] = np.nan
            estimate_skeleton.poses['RMCP2'][mask[:, 5]] = np.nan
            estimate_skeleton.poses['RMCP5'][mask[:, 5]] = np.nan

        if args.filter_lowConf_legs:
            kpt_names = bad_kpts_data['kpt_names']  # list of kpt names corresponding to the mask columns
            conf_2d = bad_kpts_data['confidence_2d']  # shape: (frame_num, kpt_num)
            threshold = 5.8
    ...
    if args.type == 'hand':
        ...
        if args.dynamic_plot_range:
            ...
            for segment_id in tqdm(range(segment_count)):
                start_frame = segment_id * 243
                end_frame = start_frame + 243
                # 3D plot range
                xyz_pelvis = estimate_skeleton.poses['PELVIS'][start_frame:end_frame].reshape(-1, 1, 3)
                xyz_centered = estimate_skeleton.points[start_frame:end_frame] - xyz_pelvis

                # Max absolute displacement from pelvis in any direction
                abs_max = np.nanmax(np.abs(xyz_centered), axis=(0, 1))  # shape (3,)

                # plot_range needs to cover the furthest point from pelvis, times 2 (both sides)
                plot_range = np.nanmax(abs_max) * 2.0 * 0.9
                estimate_skeleton.plot_3d_pose(args.output_frame_folder, start_frame=start_frame, end_frame=end_frame, coord_system="camera-px", plot_range=plot_range, mode=args.plot_mode, center_key='PELVIS', downsample=downsample)

                # if False:
                #     print(start_frame, plot_range)
                #     estimate_skeleton.plot_3d_pose_frame(frame=frame, coord_system="camera-px", plot_range=plot_range, mode="camera_side_view", center_key='PELVIS')
                #     estimate_skeleton.plot_3d_pose_frame(frame=frame, coord_system="camera-px", plot_range=plot_range, mode="paper_view", center_key='PELVIS')
                        #
        # else:
