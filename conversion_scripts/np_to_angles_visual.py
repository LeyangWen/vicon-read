...
if __name__ == '__main__':
    ...
    if args.filter_bad_support_kpts or args.filter_lowConf_legs:
        ...
        if args.filter_bad_support_kpts:
            ...
            # shoulder rotation mask: True if shouulder or elbow is bad (true)
            L_shoulder_rot_mask = np.logical_or(mask[:, 0], mask[:, 2])
            R_shoulder_rot_mask = np.logical_or(mask[:, 1], mask[:, 3])
            L_shoulder_abd_mask = mask[:, 0]
            ...
            estimate_ergo_angles['right_shoulder'].rotation[R_shoulder_rot_mask] = np.nan
            estimate_ergo_angles['left_shoulder'].abduction[L_shoulder_abd_mask] = np.nan
        ...
        if args.filter_lowConf_legs:
            ...
            both_knee_angle_mask = left_knee_mask & left_ankle_mask & right_knee_mask & right_ankle_mask
            both_knee_angle_mask = both_knee_angle_mask.reshape(-1)
            estimate_ergo_angles['left_knee'].flexion[both_knee_angle_mask] = np.nan
            estimate_ergo_angles['right_knee'].flexion[both_knee_angle_mask] = np.nan
